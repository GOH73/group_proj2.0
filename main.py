...
def compute_similarity(query_features, image_features):
    query_sift, query_color = query_features
    image_sift, image_color = image_features

    # Compute SIFT similarity
    sift_similarity = cosine_similarity(query_sift, image_sift)[0][0]

    # Compute color histogram similarity
    color_similarity = cosine_similarity(np.array([query_color]), np.array([image_color]))[0][0]

    # Combine both similarities (you can adjust the weights based on preference)
    similarity = (sift_similarity + color_similarity) / 2
